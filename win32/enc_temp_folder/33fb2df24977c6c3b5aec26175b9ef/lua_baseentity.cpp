...
*  Purpose : Returns true if a monster can use a particular spell given a job and level
*  Example : if (player:canUseSpellWith(528)) then
*  Notes   :
************************************************************************/
inline int32 CLuaBaseEntity::canUseSpell(lua_State* L)
{
    DSP_DEBUG_BREAK_IF(m_PBaseEntity == nullptr);

    auto isNumber = lua_isnumber(L, 1);

    DSP_DEBUG_BREAK_IF(lua_isnil(L, 1) || !isNumber);

    auto caster = (CBattleEntity*)m_PBaseEntity;
    uint16 spellID = (uint16)lua_tointeger(L, 1);

    lua_pushboolean(L, (spell::CanUseSpellWith((SpellID)spellID, caster->GetMJob(), caster->GetMLevel()) != 0));
...
inline int32 CLuaBaseEntity::canLearnSpell(lua_State *L)
{
    ...
    auto isNumber = lua_isnumber(L, 1);
    auto number = (uint16)lua_tointeger(L, 1);
...
Lunar<CLuaBaseEntity>::Register_t CLuaBaseEntity::methods[] =
{
    ...
    LUNAR_DECLARE_METHOD(CLuaBaseEntity,canUseSpell),
